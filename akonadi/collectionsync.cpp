...
public:
    ...
    RemoteId(const QStringList &ridChain):
        ridChain(ridChain)
    {
    }
...
public:
    ...
    void createLocalFetchJob(const Collection &parentCollection)
    {
        ...
        job->setProperty(PARENTCOLLECTIONRID, QVariant::fromValue(remoteIdForCollection(parentCollection)));
        job->setProperty(PARENTCOLLECTION, QVariant::fromValue(parentCollection));
    ...
    void localCollectionFetchResult(KJob *job)
    {
        ...
        // that means we are done and we can start writing into Akonadi
        if (pendingJobs == 0) {
            qDebug() << "=== Local Collections Fetch DONE!";
            localListDone = true;
            execute();
        }
    }
    ...
    */
    ...
    void createLocalCollectionResult(KJob *job)
    {
        ...
        //qDebug() << "CREATED" << newLocalRID << "(parent " << newLocal.parentRemoteId() << ")";
        ...
        // collection
        if (!remoteCollections.isEmpty() && remoteCollections.contains(newLocalRID)) {
            const QList<Collection> collections = remoteCollections.take(newLocalRID);
            Q_FOREACH (Collection col, collections) {
    ...
    */
    ...
    void done()
    {
        qDebug() << "========= CollectionSync finished in" << mTime.elapsed();
        q->emitResult();
    }
...
{
    qDebug() << "========= CollectionSync started";
    d->mTime.start();
    d->createLocalFetchJob(Collection::root());
}
